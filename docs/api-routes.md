...
| POST /api/portal/dictionary-lookup | guard | 20/min | 词典·AI 查词/详情补全:`mode=lookup|enrich`;本地 miss 或详情页懒加载例句/助记/词根/搭配 |
...
| POST /api/portal/drive | guardAiRoute (300/min) + Google OAuth (`drive.appdata`+`drive.file`) — `{action:'session'|'beginResumable'|'putChunk'|'uploadGzip'|'openDownload'|'readChunk'}` 代传/分片代下「宝盒备份」(手机不直连 Google;大包恢复不经整包 JSON) |
