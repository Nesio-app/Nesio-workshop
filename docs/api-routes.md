...
| POST /api/portal/drive | guardAiRoute (300/min) + Google OAuth (`drive.appdata`+`drive.file`) — `{action:'session'|'beginResumable'|'putChunk'|'uploadGzip'}` 代传到「宝盒备份」(手机不直连 Google) |
