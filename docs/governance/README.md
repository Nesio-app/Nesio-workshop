...
| 🟡 仅报告聚合 | 17 | 已进 report:modules,但没有任何 UI 消费(可见性缺口) |
| 🟡 休眠种子 | 3 | 为未来治理/经济控制预留,尚未接线 |
| 🔴 契约漂移 | 1 | 契约与运行时不一致,给人假约束错觉,需处理 |
| 🔴 死代码 | 1 | 接了但零消费,可净删 |

## 怎么读这本书

- [治理地图](status-map.md) —— 全部治理面一张表,按层分组,带状态。**先看这页。**
- [模块数据总线](data-bus.md) —— 模块之间的数据归属与连接(以及为什么 27 个 data-key 是孤立的)。
