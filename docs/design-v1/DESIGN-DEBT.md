...
- [ ] **今天页时间线 · 置顶卡上轨**:批次 108 把心情 + 折叠要紧事串成一条连续竖轨(用户圈出
  「浮卡没接线」已修)。但**日历事件的 PinnedAttentionCard**(「现在」最紧那张富卡,带录音钮等)
  还是张卡、没做成轨上节点 —— 有日历事件被置顶时它会断轨。设计稿里「现在」也是裸节点(实心点),
  要把 pinned 卡也改成轨上节点(实心点 + 时标 kicker)。有日历数据的用户才会遇到。
- [ ] **proactive 卡日间浅蓝底**:`.nesio-proactive-card`(globals ~14865)日间硬编码浅蓝
  `rgba(232,242,252)`,在灰粉皮肤上偏蓝(夜间已在 106 修,日间未修)。换中性/皮肤色。
