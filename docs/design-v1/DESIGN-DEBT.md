...
- [ ] **夜间皮肤不跟色卡**:莫兰迪 4 色卡只覆写日间 token(设计规范补丁明确「夜间未动」)。
  夜间规则里散着一批硬编码亮蓝 `#96c7f3` / `rgba(88,140,227,…)`(globals.css 约 30 处,
  文字/描边/active 态)。要夜间也跟皮肤,得给 4 套各加一组
  `html[data-portal-theme="night"][data-palette="X"]` token + 把那些硬编码蓝换成 token。
  一块独立的中等工程。
