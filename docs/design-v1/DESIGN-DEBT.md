...
- [x] **批次 130 · 任务 AI 细化重画为克制清单(图2)**:FocusCardDetail —— ①去掉亮蓝大按钮(粉碎任务
  实心蓝 → accent-soft 描边「拆成小步」)和层层套框(row 去 surface-2 底 → 步骤=干净白卡,子步骤卡内);
  ②完成勾 蓝→绿(--status-go)+ 完成名划掉(line-through);③子步骤细线缩进(border-left 竖线);
...
- [x] **批次 133 · 收纳卡片换成有用信息(设计「收纳·打开」)**:InventorySheet 列表重画 ——
  ①**去掉抽象方块统计**:总数收成标题旁一行小字(N 件 · 估值 $X · N 未归位),不占整块;标题去 📦;
  ②每条物品卡显示三样有用的:**预览图**(色块占位,previewColor 确定性取色;真图缩略待接)+ **一级存放
  位置**(IconMapPin + location,未归位=accent 提示)+ **最后更新多久前 · 来源**(IconClock + relativePastLabel
  + 拍照/手记);③「物品·最近更新在前」段标;④底部「常用标签」chip(点筛该标签)。预览实测(牛奶):
  标题旁「1 件·1 未归位」+ 色块卡 + 未归位·点开设位置 + 4小时前更新·手记 + 已过期。
  **仍待**:统计/卖闲置/导入/记一件 按钮的 emoji(📊💰📄）未线性化;按位置/按分类 显式模式;真图缩略加载。

## 设置页(批次 134 起)

- [x] **批次 134 · 通用·精简(设计「去掉语气 + 主动提醒程度」)**:GeneralSheet 删掉「偏好」折叠组 +
  语气(tone/toneOpts/示例)+ 提醒程度(reminders/levelOpts);开关组直接铺开:每日 AI 图文日报 /
  触感反馈 / 记忆自动定位(hint 精简到设计文案),再接 外观(日/随系统/夜)+ 语言。tone/interrupt/prefsOpen
  state 留着未用(build 容忍;route allowedSettingsKeys 的 coachStyle 不动,契约不红)。门禁全绿。
  **仍待(设计完整重排)**:①账户页(登录 邮箱/套餐 + 修改密码 + 购买 + 危险区 删账号/退出);
  ②个人资料页精简(头像水晶球 + 昵称/头像,去「宝盒对你的了解」——现已不存在);③设置主页清爽分组
  (个人资料/通用/账户);④开关用真 toggle 开关样式(现 ✓/○)。

## 对话页(念念,批次 135 起)

- [x] **批次 135 · 长按菜单补「以此为题继续问」(设计对话页)**:BubbleMenu 从「复制/存入记忆」补齐到
  设计的三项:**存入记忆 / 复制 / 以此为题继续问**(顺序也按设计:存入记忆在前);图标从 ⎘/＋ 换线性
  (IconBox/IconLink/IconHelpCircle)。以此为题继续问 = 把这条内容截断成「关于「…」,」预填输入框 + 聚焦,
  用户接着追问,不跳页。契约不红。**仍待(设计对话页整体)**:①端上简答(免费)/深问(Pro 云)显式开关
  (分层门 canUsePaidCloudAi 已有,缺 UI 切换 + 头部模式副标)；②念念头部品牌 + 端上/深问 副标；
  ③消息气泡的模式徽章(端上答的·免费 / 深问·云端·Pro）。

## 心情·情绪趋势(批次 136)

- [x] **批次 136 · 情绪趋势 sheet(设计「心情·情绪趋势」)**:新建 MoodTrendSheet —— 首页心情第一拍
  「看趋势」(nesio-open-mood-trend)从「打开洞察 tab」改为专门的情绪趋势视图。①柱状图:一柱一天
  (一~今天),**柱高=能量**(蔫/中/满电→30/58/88%)、**柱色=当天心情**(12 色定稿 var(--emotion-<id>)),
  今天高亮;②↑ 比上期回暖/降/平稳(近半窗 vs 前半窗能量均值差);③念念一句(衬线嗓音)这期更多是
  X 和 Y;④情绪分布(色点+条+天数,取窗口内所有心情节点计数);⑤本周/本月/今年可切(月/年扩窗、
  柱按 step 聚合)。数据 = health_state+feeling 节点的 attributes.emotion/energyLevel;无数据给诚实空态。
  预览实测:注入样例后柱图/念念句/分布全对齐 mockup。TodayFeed dynamic import + moodTrendOpen state 接入。
