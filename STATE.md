...
- **UI QA 报告修复第一批(2026-07-29;报告约 50 条,先修阻断与误导)**:
  ...
  **未修(下一批)** —— 性能卡顿 10-45s(速记提交/2328 条列表/切页,疑云同步主线程
  gzip + 大列表无虚拟化,需专项);jot 草稿污染「关注chong」/ 积分无故 +150 / rose 主题
  静默切换(需复现);死按钮群:物品页(打开物品管理/卖闲置/分类行)、衣橱卡片详情、
  关系联系人行与「记给某人」、日程日历/邮件行、剧场播放、足迹地址链接与标记位置、
  健身「练过的/身体数据」卡;数据质量群:洞察 donut 6639≠6343、足迹 11 地点 vs 2 到访、
  库存黄瓜双分类、衣橱毯子归外套、血糖 TIR 98.5% vs 峰值 15.3、睡眠 21.5h/97.3h 区间、
  身高 7 个月 ±6cm、摄入热量超自标区间(多为 Apple Health 解析/聚合口径,需专项);
  flomo 导入 markdown 图片语法未渲染(2191 条)、** 加粗未渲染、#生命维度系统 标签
  是用户 flomo 数据但呈现应清理;双击才生效的全局点击问题;镜子页 Esc 冒泡关全层;
  Lab「Pro 解锁(测试)」开关是否下线待用户拍板;UTC 日键全仓仍有 ~50 处待系统清理。
