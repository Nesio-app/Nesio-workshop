/**
 * POST /api/portal/dictionary-lookup — 云 AI 英汉/汉英查词(可选兜底)。
 * 入:{ query, locale? };出:{ ok, entry: DictEntry }。
 * 本地词库 miss 且用户打开「AI 查词」时由 DictionarySheet 调用。
 */
...
function clampEntry(raw: unknown, query: string): DictEntry | null {
  ...
  return { word: word.toLowerCase(), headword, phonetic, senses, examples };
}
...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch(() => null) as { query?: unknown; locale?: unknown } | null;
...
规则:1-4 条释义;准确为主;不确定时在 en 里注明;不要编造生僻义。`;
  ...
  try {
    ...
    return NextResponse.json({ ok: true, entry, source: 'ai' });
  } catch (err) {
