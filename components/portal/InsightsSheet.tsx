...
type MainTab = 'reflection' | 'growth' | 'montage' | 'health' | 'fitness' | 'timeline' | 'meetings' | 'finance' | 'relationships' | 'tesla' | 'living';
...
export default function InsightsSheet({ onClose, canUsePrivateData = false, initialTab }: { onClose: () => void; canUsePrivateData?: boolean; initialTab?: MainTab }) {
  ...
  const tabLabel = (t: MainTab): string =>
    t === 'reflection' ? L(dict, '洞察', 'Insights') : t === 'growth' ? L(dict, '成长', 'Growth') : t === 'montage' ? L(dict, '小剧场', 'Films') : t === 'health' ? L(dict, '健康', 'Health') : t === 'fitness' ? L(dict, '健身', 'Fitness') : t === 'timeline' ? L(dict, '足迹', 'Places') : t === 'finance' ? L(dict, '财务', 'Finance') : t === 'relationships' ? L(dict, '关系', 'People') : t === 'meetings' ? L(dict, '会议', 'Meetings') : t === 'tesla' ? L(dict, '车 · Tesla', 'Car · Tesla') : L(dict, '多面镜', 'Mirror');
      ...
      <div className="nesio-insights-body">
        {showHub ? (
          <div className="nesio-insights-hub">
            {(['reflection', 'growth', 'montage', 'health', 'fitness', 'timeline', 'meetings', 'finance', 'relationships', 'tesla', 'living'] as MainTab[]).filter(tabEnabled).map((t) => (
              <button
                key={t}
                type="button"
