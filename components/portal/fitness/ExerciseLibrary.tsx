...
import {
  loadExerciseCatalog, filterCatalog, CATALOG_EQUIP_LABEL, CATALOG_PART_LABEL,
...
export default function ExerciseLibrary({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [catStatus, setCatStatus] = useState<'idle' | 'loading' | 'error'>('idle');
  const [q, setQ] = useState('');

  // 切到「全部」时懒加载扩展库(1324)。失败有显式失败态 + 重试。
  useEffect(() => {
    if (mode !== 'all' || catalog || catStatus === 'loading') return;
    let alive = true;
    setCatStatus('loading');
    loadExerciseCatalog()
      .then((doc) => { if (alive) { setCatalog(doc.exercises); setCatStatus('idle'); } })
          ...
          {mode === 'core' ? (
            ...
            <>
              <input
                ...
                placeholder={L(dict, '搜动作 / 部位 / 器械（英文）', 'Search name / part / gear')}
              ...
              {catalog && (() => {
                const cl = filterCatalog(catalog, q).slice(0, 300);
                return (
                  <>
                    <p className="nesio-settings-option-hint" style={{ marginTop: 0 }}>
                      {L(dict, `${filterCatalog(catalog, q).length} 个匹配${filterCatalog(catalog, q).length > 300 ? '（显示前 300）' : ''}`, `${filterCatalog(catalog, q).length} match${filterCatalog(catalog, q).length > 300 ? ' (first 300)' : ''}`)}
                    </p>
                    <div className="nesio-xlib-cards">
                      {cl.map((ex) => {
                        const expanded = openId === ex.id;
