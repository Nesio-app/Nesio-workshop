  ...
  return (
    ...
    >
      ...
      <div className="nesio-proactive-card-inner">
        ...
        {/*
         * ✕ 加回来(2026-07-31,用户实测「向左拉还是拉不动」)。
         *
         * 批次 33 把按钮全撤、只留手势。之后手势被实测打回**三次**
         * (批次 166 的注释里还留着「用户实锤向右滑动还是失败」)。
         * 这一次我没能从代码里定位左滑为什么不跟手 —— 两个方向的代码是对称的,
         * 而右滑在同一台机器上是好的。既然定不出根因,就不该再调一次阈值交差:
         * 那等于拿用户的下一次实测当调试器。
         *
         * 手势留着(能用的时候更快),但它不能是**唯一**的出口。
         * 一个只能靠手势关掉的卡片,手势一失灵就是永远关不掉 ——
         * 这正是仓里那条「每个提示都要有出口」说的事。
         */}
        <button
          type="button"
          className="nesio-proactive-card-dismiss"
          aria-label={L(dict, '稍后再说', 'Later')}
          title={L(dict, '稍后再说', 'Later')}
