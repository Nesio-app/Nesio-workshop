      ...
      <div className="nesio-nd-photo-add">
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem', flexWrap: 'wrap' }}>
          ...
          <button type="button" onClick={() => void cancelChore(phase.familyId)} disabled={cancelBusy}
            style={{ border: 'none', background: 'transparent', color: 'var(--status-risk)', fontSize: 'var(--text-sm)', cursor: 'pointer', padding: '0.2rem 0.4rem' }}>
  ...
  return (
    <div className="nesio-nd-photo-add" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.3rem' }}>
        {([['once', t('一次', 'Once')], ['daily', t('每天', 'Daily')], ['weekly', t('每周', 'Weekly')]] as const).map(([k, label]) => (
          <button key={k} type="button" onClick={() => setRepeat(k)}
            style={{
              flex: 1, padding: '0.3rem 0.4rem', borderRadius: 'var(--radius-sm)', fontSize: 'var(--text-sm)', cursor: 'pointer',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        />
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', fontSize: 'var(--text-xs)', color: 'var(--portal-muted)', cursor: 'pointer', whiteSpace: 'nowrap' }}>
      ...
      <button type="button" onClick={() => setPhase({ s: 'idle' })}
        style={{ alignSelf: 'flex-start', border: 'none', background: 'transparent', color: 'var(--portal-muted)', fontSize: 'var(--text-xs)', cursor: 'pointer', padding: '0.2rem 0' }}>
        {t('收起', 'Close')}
      </button>
    </div>
  );
