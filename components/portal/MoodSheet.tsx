            ...
            <div style={{ display: 'flex', gap: 'var(--space-1)' }}>
              {([['write', L(dict, '写一篇', 'Write')], ['history', L(dict, '历史', 'History')]] as const).map(([id, label]) => (
                <button key={id} type="button"
                  ...
                  style={{ fontSize: 'var(--text-xs)', fontWeight: 600, padding: 'var(--space-1) var(--space-3)', borderRadius: 999, border: 'none', cursor: 'pointer', background: journalTab === id ? 'var(--portal-blue-deep)' : 'rgba(88,140,227,0.1)', color: journalTab === id ? '#fff' : 'var(--portal-blue-deep)' }}>
                  {label}
                </button>
              ))}
            </div>
          ...
          {journalTab === 'write' ? (
            ...
            <>
              ...
              <div style={{ maxHeight: '46vh', overflowY: 'auto', paddingRight: 2 }}>
                {filtered.length === 0 && (
                  <p style={{ fontSize: 'var(--text-sm)', color: 'var(--portal-muted)', textAlign: 'center', padding: 'var(--space-5) 0' }}>
                ...
                {[...byDay.entries()].map(([day, list]) => (
                  <div key={day} style={{ marginBottom: 'var(--space-3)' }}>
                    {/* 时间线:日期节点 + 竖线 */}
                    <p style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--portal-blue-deep)', margin: '0 0 var(--space-1)' }}>
                      <IconBook size={13} /> {day}
                    </p>
                    <div style={{ borderLeft: '2px solid var(--portal-line)', marginLeft: 6, paddingLeft: 12, display: 'grid', gap: 'var(--space-2)' }}>
                      {list.map((e) => {
                        ...
                        return (
                          <button key={e.id} type="button"
                            ...
                            style={{ textAlign: 'left', background: 'rgba(88,140,227,0.05)', border: '1px solid var(--portal-line)', borderRadius: 'var(--radius-sm)', padding: 'var(--space-2) var(--space-3)', cursor: 'pointer' }}>
                            <p style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 'var(--text-overline)', color: 'var(--portal-muted)', margin: '0 0 var(--space-1)' }}>
