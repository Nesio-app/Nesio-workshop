...
import { formatMoney, investmentAccountIds, type BankTx, type Holding } from '@/lib/portal/bank-tx';
...
export default function InvestPane({ txs, holdings, nwSeries, currency, dict }: {
  txs: BankTx[]; holdings: Holding[]; nwSeries: NetWorthSnapshot[]; currency: string; dict: string;
}) {
  ...
  const empty = !daily && ytd.dividends <= 0 && ytd.interest <= 0 && hs.length === 0 && !checkup;
          ...
          <>
            ...
            {(ytd.dividends > 0 || ytd.interest > 0) && (
              <>
                ...
                <p className="nesio-fin-alert-note" style={{ textAlign: 'left' }}>{L(dict, `股利 ${formatMoney(ytd.dividends, currency)} · 利息 ${formatMoney(ytd.interest, currency)}`, `Dividends ${formatMoney(ytd.dividends, currency)} · interest ${formatMoney(ytd.interest, currency)}`)}</p>
