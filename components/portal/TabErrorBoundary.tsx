    ...
    if (this.state.error) {
      ...
      return (
        <div className="nesio-analytics-tab" style={{ padding: 'var(--space-4)' }}>
          ...
          <p style={{ fontSize: 'var(--text-sm)', color: 'var(--portal-muted)', margin: 'var(--space-2) 0 0', lineHeight: 1.6 }}>
            别的页面不受影响。把下面这段截图发我,我就能定位:
          </p>
          <pre style={{ margin: 'var(--space-2) 0 0', padding: 'var(--space-2)', fontSize: 'var(--text-overline)', lineHeight: 1.4, color: 'var(--status-risk)', background: 'var(--status-risk-soft)', borderRadius: 'var(--radius-sm, 12px)', whiteSpace: 'pre-wrap', wordBreak: 'break-word', maxHeight: '10rem', overflow: 'auto' }}>
            {msg}
          </pre>
          <button type="button" className="nesio-ob-primary-btn" style={{ marginTop: 'var(--space-2)', width: '100%' }} onClick={() => this.setState({ error: null })}>
            重试
          </button>
        </div>
      );
