...
function loadTs(rel) {
  ...
  const mod = { exports: {} };
  vm.runInNewContext(js, { module: mod, exports: mod.exports, require: () => ({}), console, Date, Math, Number, Array, Object, String, RegExp, parseFloat, process: { env: {} }, fetch: () => Promise.reject(new Error('no-net')) });
  return mod.exports;
}
...
// 每条字段齐 + 绝大多数含中文分步
let zh = 0; let zhName = 0;
for (const e of doc.exercises) {
  assert.ok(e.id && e.name, `${e.id} 有 id/name`);
  assert.ok(typeof e.nameZh === 'string' && e.nameZh, `${e.id} 有 nameZh`);
  ...
  if (/[一-鿿]/.test(e.nameZh)) zhName += 1;
}
assert.ok(zh / doc.exercises.length > 0.9, `>90% 含中文分步,实际 ${zh}/${doc.exercises.length}`);
assert.ok(zhName / doc.exercises.length > 0.8, `>80% 中文名,实际 ${zhName}/${doc.exercises.length}`);
