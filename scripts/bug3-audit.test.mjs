...
const ITEMS = [
  ...
  ['47', 'p18 出发/到达给离线枢纽下拉', () => /function HubField/.test(F.tripTimeline) && /searchTravelHubs/.test(F.hubs) && /searchTravelHotels/.test(F.tripTimeline)],
  ['48', 'p18 离线机场/车站/酒店/航线数据包', () => exists('public/data/travel-hubs/hubs.json') && exists('public/data/travel-hotels/hotels.json') && exists('public/data/travel-routes/routes.json') && /ensureTravelHubsLoaded/.test(F.hubs)],
  ...
  ['55', 'p23 预算可编辑 + 清零自愈', () => /setCategoryBudget/.test(F.trips) && /setTripBudgetTotal/.test(F.trips) && /clearTripBudget/.test(F.trips) && /MAX_REASONABLE_PRICE/.test(F.trips)],
  ...
  ['60', 'p27 节点可删(待办/航班/酒店等)', () => /function DeleteNodeButton/.test(F.tripNodes) && /function TodoDetail/.test(F.tripNodes) && C.tripNodes.includes('删掉这条')],
