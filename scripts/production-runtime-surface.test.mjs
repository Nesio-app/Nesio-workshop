...
assert.match(authStartRoute, /authMode/, 'auth start must distinguish login from register mode.');
assert.match(authStartRoute, /create_user:\s*shouldCreateUser/, 'auth start must not always create users for email login.');
assert.match(authStartRoute, /requestSupabaseOtp\(\{ email, redirectTo, authMode \}\)/, 'auth start must send the trimmed email and mode to Supabase OTP.');
assert.match(authStartRoute, /requestSupabaseOtp\(\{ phone, redirectTo, authMode \}\)/, 'auth start must send the trimmed phone and mode to Supabase OTP.');
