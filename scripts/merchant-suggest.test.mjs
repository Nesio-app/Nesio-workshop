...
// 1. 精确 token 投票(原行为):Wegmans/Trader Joe's → Food,"Joe's Market" 命中 joe
{
  ...
  assert.equal(r.category, 'Food', '精确 token 命中(joe)');
}
...
// 2. 变体模糊命中:McDonalds 纠正过 → MCDONALD'S #123(mcdonald~mcdonalds)
{
  ...
  assert.equal(r.category, 'Food', '变体经模糊命中');
...
// 3. 编辑距离 1:walmart ~ wallmart(插入一位)
{
  ...
  assert.equal(suggestCategory('WALLMART STORE', rules).category, 'Shopping', '编辑距离 1 命中');
}
