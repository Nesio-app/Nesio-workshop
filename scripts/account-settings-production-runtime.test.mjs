...
for (const labelKey of [
  'providerGeminiLabel',
  'providerGoogleCalendarLabel',
  'providerCloudDatabaseLabel',
  'authEmailLabel',
  'authGoogleLabel',
  'authWechatLabel',
  'authPhoneLabel',
]) {
  assert.ok(source.includes(labelKey), `AccountSettings must display localized production status label key: ${labelKey}`);
}
...
assert.ok(source.includes('observationPushEnabled: normalizeObservationPushEnabled'), 'AccountSettings must normalize cloud observation push preferences');
assert.ok(source.includes('setObservationPushEnabled(next.observationPushEnabled)'), 'AccountSettings must apply cloud observation push preferences to runtime state');
assert.ok(source.includes('localStorage.setItem(OBSERVATION_PUSH_KEY, String(next.observationPushEnabled))'), 'AccountSettings must mirror cloud observation push preferences to local fallback storage');
assert.ok(source.includes('observationPushEnabled: next'), 'AccountSettings observation push toggle must write through to cloud profile settings');
