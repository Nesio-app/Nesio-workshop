...
// 真 learner 底座 + 真 ranker(共享同一 window/localStorage)
const learnerMod = run(transpile('../lib/portal/learning/learner.ts'), () => ({}));
const mirrorCalls = [];
const rankerMod = run(transpile('../lib/platform/guidance-engine/guidance-ranker.ts'), (p) =>
  p.endsWith('/learner') ? learnerMod : { learnFromFeedback: (type, fb) => mirrorCalls.push(`${type}:${fb}`) },
);
...
learnerMod.emitFeedback({ verdict: 'useful', cardId: 'card1', at: '2026-01-01T00:00:00.000Z' });
...
// 3. 无暂存特征的卡不学
learnerMod.emitFeedback({ verdict: 'useful', cardId: 'ghost', at: '2026-01-01T00:00:01.000Z' });
