...
const evidenceSummary = {
  ...
  const report = {
    ...
    domainCapabilityTaxonomyVersion: registry.domainCapabilityTaxonomy.version,
    frontstageDomainCount: registry.domainCapabilityTaxonomy.frontstageDomains.length,
    signalInputContractVersion: registry.toolManifest.signalInputContract.version,
    signalInputSurfaceCount: registry.toolManifest.signalInputContract.summary.surfaceCount,
    signalInputSensitiveSurfaceCount: registry.toolManifest.signalInputContract.summary.sensitiveSurfaceCount,
    signalInputExternalSurfaceCount: registry.toolManifest.signalInputContract.summary.externalSurfaceCount,
    signalInputWritesSignalSurfaceCount: registry.toolManifest.signalInputContract.summary.writesSignalSurfaceCount,
    signalInputWarningCount: registry.toolManifest.signalInputContract.summary.warningCount,
    platformCapabilityMaterialCount: registry.capabilityMaterialSummary.filter(
      (item) => item.primaryDomain === 'platform',
    ).length,
  ...
  domainCapabilityRegistry: {
    version: registry.domainCapabilityTaxonomy.version,
    taxonomy: registry.domainCapabilityTaxonomy,
    signalInputContract: registry.toolManifest.signalInputContract,
    governanceResolver: {
      version: GOVERNANCE_RESOLVER_VERSION_V1,
      order: registry.domainCapabilityTaxonomy.governanceResolverOrder,
      stages: registry.domainCapabilityTaxonomy.governanceResolverOrder.map((stage, index) => ({
        stage,
        order: index + 1,
      })),
      policy: {
        safetyGateFirst: true,
        approvalGatePrecedesPaywall: true,
        entitlementAfterSafety: true,
        quotaBeforeCost: true,
        auditLogAlwaysRequired: true,
        executesActions: false,
        runtimeSideEffects: false,
        reportOnly: true,
      },
    },
    domainSummary: registry.domainSummary,
    capabilitySummary: registry.capabilitySummary,
    capabilityMaterialSummary: registry.capabilityMaterialSummary,
    materialLibrarySummary: registry.materialLibrarySummary,
    moduleIndex: Object.fromEntries(registry.modules.map((module) => [module.id, {
      moduleId: module.id,
      label: module.name,
      toolLifecycle: module.toolManifest?.toolLifecycle,
      launchStatus: module.toolManifest?.launchStatus,
      prodExposure: module.toolManifest?.prodExposure,
      primaryDomain: module.toolManifest?.primaryDomain,
      primaryDomainZhLabel: module.toolManifest?.primaryDomainZhLabel,
      primaryDomainEnglishLabel: module.toolManifest?.primaryDomainEnglishLabel,
      secondaryDomains: module.toolManifest?.secondaryDomains || [],
      visibility: module.toolManifest?.visibility,
      gateLevel: module.toolManifest?.gateLevel,
      providedCapabilities: module.toolManifest?.providedCapabilities || [],
      requiredCapabilities: module.toolManifest?.requiredCapabilities || [],
      salvageStatus: module.toolManifest?.salvageStatus,
