...
export async function applyIFS(sourceText: string, locale: string, focus?: string): Promise<IFSResult | null> {
  ...
  const t = await chatText(
    ...
    // 2026-07-29 标注(Bug4 P2「落到身体的提示,似乎没变过」):原来 prompt 里给了
    // 「如把手放在心口、深呼吸三次」这个例子,模型每次都照抄它 —— 例子本身就是病根。
    // 改成:不给例子,反过来要求动作**贴着这次的情绪与场景**,并明确点名别再用那两个动作。
    `- action:一件 2 分钟内可执行的躯体微动作,必须**贴着这次的情绪和场景**选(不同的情绪配不同的动作:` +
    `紧绷可以松开下颌/抖手臂、发闷可以开窗深吐气、心慌可以脚踩实地面数五样看得见的东西…)。` +
    `不要每次都用同一个动作,尤其别再用「把手放在心口」和「深呼吸三次」。\n` +
