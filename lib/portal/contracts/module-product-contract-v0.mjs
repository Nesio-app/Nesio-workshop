...
const PRODUCT_COPY_BY_MODULE = Object.freeze({
  ...
  plan: Object.freeze({
    launchCohort: 'first_launch',
    primaryUser: '需要低摩擦拆分任务、提醒和恢复行动的首发用户',
    openMoment: '用户卡住、分心或需要把下一步拆小的时候',
    primaryUseCase: '把待办拆成可执行的本地行动线索与轻提醒',
    primaryAction: '创建一个 FlowTask 或粉碎任务',
    publicPromise: Object.freeze(['本地待办', '粉碎任务', '轻提醒与下一步行动']),
    doNotPromise: Object.freeze(['AI 自动执行', '替用户完成外部任务', '云端任务同步', '外部日历写入']),
    riskTier: 'low',
    dataObjects: Object.freeze(['FlowTask', 'RecoveryStep']),
    firstRunPath: 'open_todo -> create_flow_task -> mark_next_action_or_reminder',
    readinessBlockers: Object.freeze(['TestFlight 前需要 Ming/Zhao 移动端 Todo 主路径验收']),
    monetizationStatus: 'free_first_launch',
    marketingClaim: '用宝盒把卡住的事拆成一个小动作。',
    appReviewNote: '首发 Todo 仅为本地任务/提醒辅助；不启用 AI 自动执行、云同步或外部日历写入。',
    privacyLabelImpact: 'local_user_content_task_notes',
  }),
...
export function resolveLaunchCohort(tool) {
  ...
  if (['inventory', 'plan'].includes(moduleId)) return 'first_launch';
  if (['reading', 'sanctuary', 'fitness', 'quiz'].includes(moduleId)) return 'sandbox';
...
export function resolveTrustBoundary(tool) {
  ...
  return {
    moduleId,
    canStoreLocalData: ['inventory', 'plan'].includes(moduleId) || launchCohort === 'sandbox',
    ...
    requiresCEOApproval,
    requiresUserConsent: requiresCEOApproval || ['inventory', 'plan'].includes(moduleId) || mentalHealthModules.includes(moduleId),
    appStoreClaimBoundary: moduleId === 'inventory'
      ? 'Only claim local Inventory / purchase-memory; no cloud, AI, payment, or external auth.'
      : moduleId === 'plan'
      ? 'Only claim local Todo / task support; no AI execution, cloud sync, or external calendar writes.'
...
export function resolveShellDisplayState(productEntry) {
  ...
  if (launchCohort === 'sandbox') {
    if (['reading', 'sanctuary'].includes(productEntry.moduleId)) {
      return {
        displayGroup: 'Today Core',
        stateKind: 'building',
        stateLabel: '建设中',
        userVisible: false,
        actionAllowed: false,
