...
export type InsightDomain = 'health' | 'finance' | 'location' | 'mood';
...
export function domainInsightsContextBlock(max = 8): string {
  ...
  const label: Record<InsightDomain, string> = { health: '健康', finance: '财务', location: '活动', mood: '心情' };
