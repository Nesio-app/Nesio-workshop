 ...
 */
import { anomaly, explainBaseline, robustStats } from './analyst-baseline.mjs';

const SEV_RANK = { go: 0, gentle: 1, risk: 2 };

/** 成功率归一:>1 视为 0–100 百分数(metrics 路由口径),≤1 视为 0–1 小数。
 *  两路消费方(契约喂小数、/api/admin/metrics 喂百分数)曾打架 → 「成功率 10000%」。 */
const okRateFrac = (v) => (v == null ? null : v > 1 ? v / 100 : v);

/** 可被基线学习的信号 → 中文标签(单位)。 */
export const SIGNAL_LABELS = {
  weekEvents: '本周活跃事件', aiCost: 'AI 成本($)', aiOkRate: 'AI 成功率',
  errTotal: '客户端报错量', orphanedKeys: '孤立 data-key',
};
/** 预警类型 → 中文标签(反馈学习/静音展示用)。 */
export const ALERT_LABELS = {
  activity_drop: '活跃度下滑', activity_spike: '活跃度上涨', ai_okrate: 'AI 成功率',
  ai_cost: 'AI 成本', client_errors: '客户端报错', gov_drift: '治理漂移',
  gov_dead: '死代码', gov_warn: '注册表告警', orphaned_keys: '孤立键增多', card_wrong: '洞察卡不准',
};
...
export function extractSignals(metrics, gov) {
  ...
  return {
    ...
    aiOkRate: okRateFrac(ai.okRate), // 统一成 0–1 小数(metrics 路由给 0–100 百分数 → 曾显示「成功率 10000%」)
...
export function buildDailyReport(metrics, gov, opts = {}) {
  ...
  const keyPoints = [
    ...
    `AI:${num(ai.calls)} 次,约 $${ai.estCostUsd ?? '—'},成功率 ${ai.okRate == null ? '—' : Math.round(okRateFrac(ai.okRate) * 100) + '%'}`,
...
/**
 * 学习状态 —— 透明地告诉你分析师「学到了什么」:每个信号当前的基线水位(中位数±σ、样本数、
 * 是否还在冷启动),以及被你反馈静音/在乎的预警类型。给「学习状态」小面板用。
 */
export function computeLearningState(history, feedbackRecords, opts = {}) {
  const minSamples = opts.minSamples ?? 10;
  const baselines = [];
  for (const [key, label] of Object.entries(SIGNAL_LABELS)) {
    const s = robustStats(seriesFrom(history, key));
    baselines.push({ key, label, n: s.n, center: s.median, sigma: s.sigma, cold: s.n < minSamples });
  }
  const fb = summarizeFeedback(feedbackRecords);
  const feedbackTypes = Object.entries(fb).map(([type, b]) => ({
    type, label: ALERT_LABELS[type] || type, usefulRate: b.usefulRate, samples: b.total, muted: b.muted,
  })).sort((a, b) => (b.samples - a.samples));
  return {
    historyDays: (history || []).length,
    baselines,
    feedbackTypes,
    muted: feedbackTypes.filter((t) => t.muted),
  };
}
