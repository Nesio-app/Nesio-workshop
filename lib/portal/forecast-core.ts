...
export type Verdict = 'adopt' | 'reject' | 'unproven';
...
/** 采纳门槛:样本够 + 明确赢过笨基线。宁可判「存疑」,不放行侥幸。 */
export const MIN_SAMPLES = 8;
export const MIN_SKILL = 0.05;

export function scoreSamples(name: string, samples: readonly Sample[]): SkillReport {
  const n = samples.length;
  if (n === 0) {
    return { name, n: 0, mae: 0, mape: 0, naiveMae: 0, skill: 0, bias: 0, p80Pct: 0, verdict: 'unproven', note: '无样本' };
  }
  ...
  else { verdict = 'adopt'; note = `比笨基线好 ${(skill * 100).toFixed(1)}%`; }
  ...
  return {
    ...
    bias: r2(signedSum / n),
    p80Pct: r2(quantile(absPcts, 0.8)),
    verdict, note,
  };
}
