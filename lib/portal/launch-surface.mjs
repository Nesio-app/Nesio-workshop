import { isAppStoreBuild, isAppStoreBlocked } from './app-build.mjs';
...
export function resolveLaunchSurfaceState(tool, context = {}) {
  ...
  // 提审构建:v1 隐藏集(财务/健康/地图/实验室/people)在这个二进制里不可达,盖过一切。
  if (isAppStoreBlocked(tool.id)) { visible = false; reason = 'appstore_hidden'; }
...
export function readLaunchSurfaceContextFromBrowser() {
  ...
  // 提审构建:彻底断 Lab 激活 —— 无视 lab 旗 + ?baohePersonal=1 URL 参数,永远 public。
  // 否则用户手打参数就进 Lab = Guideline 2.3.1「隐藏可达功能」违规。
  if (isAppStoreBuild()) return { viewerRole: 'public', testerAllowlist: [] };

  try {
    const params = new URLSearchParams(window.location.search);
    // 显式退出 Lab:写 '0' 持久化(本仓个人版默认 Lab 开,删 key 会退回默认开)。
    if (params.get('baohePublic') === '1' || params.get('baohe_public') === '1') {
      window.localStorage.setItem('baohe_personal_lab', '0');
    ...
    // 本仓 = 个人全功能版:默认 personal_lab;只有显式 '0'(设置→Lab 关 /
    // ?baohePublic=1)才走公开/tester 分支 —— 用于预览公开形态。
    // ?baohePersonal=1 等参数保留 = 从退出态一键回到全开。
    const labParamOn = params.get('baohePersonal') === '1' ||
      params.get('baohePersonalLab') === '1' ||
      params.get('baohe_personal_lab') === '1' ||
      params.get('baoheLab') === '1' ||
      params.get('baohe_lab') === '1';
    const labOptedOut = window.localStorage.getItem('baohe_personal_lab') === '0';
    const isPersonalLab = labParamOn || !labOptedOut;
    if (isPersonalLab) {
