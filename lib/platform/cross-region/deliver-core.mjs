...
export function gateDeliverables(rels, opts = {}) {
  ...
  // 排序:有 P3 bandit ranker 用它(学「你在乎哪类跨区」),否则按 |strength|(冷启动=旧序)
  const ranked = typeof opts.rankFn === 'function'
    ? opts.rankFn(filtered)
    : filtered.slice().sort((a, b) => Math.abs(b.strength) - Math.abs(a.strength));

  return ranked
    .slice(0, cfg.budgetK)
    .map((r) => ({ ...r, cooldownBucket: strengthBucket(r.strength, cfg.materialDeltaRho) }));
}
